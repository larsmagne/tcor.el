     ...
     (cl-loop
      ...
      do (cl-loop for line across (plist-get spage :text_lines)
		  do (cl-loop
			...
			(when (and (= char-num line-length)
				   (or (eq (plist-get char :bbox_valid) t)
				       (null prev)))
			  (setq prev char))
			(when word
...
(defun tcor-surya-sharded (instance instances)
  (dolist (surya (directory-files-recursively "~/src/kwakk/magscan/" "\\`results.json\\'"))
    (when (= (mod (string-to-number (sha1 surya) 16) instances) instance)
      (tcor-possibly-parse-surya surya)))
  (kill-emacs))
...
(defun tcor-possibly-parse-surya (surya)
